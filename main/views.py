...
def topic_entries(request, topic_id):
    ...
    return render(request, 'main/topic_entries.html', context)
...
def new_entry(request, topic_id):
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
            ...
            new_entry.save()
            return redirect('main:topic_entries', topic_id = topic_id)
...
def edit_entry(request, entry_id):
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('main:topic_entries', topic_id=topic.id)
...
@login_required
def delete_topic_page(request, topic_id):
    topic = Topic.objects.get(id=topic_id)
    if topic.owner != request.user:
        raise Http404
    return render(request, 'main/delete_topic.html', {'topic': topic})
...
@login_required
def delete_entry_page(request, entry_id):
    entry = Entry.objects.get(id=entry_id)
    topic = entry.topic
    if topic.owner != request.user:
        raise Http404
    return render(request, 'main/delete_entry.html', {'topic': topic, 'entry': entry})
